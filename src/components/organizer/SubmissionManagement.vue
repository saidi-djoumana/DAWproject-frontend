<template>
  <div class="page-container">
    <div class="container">
      ...
      <div v-else class="table-container" v-if="filteredSubmissions.length">
        <table>
          <thead>
            <tr>
              <th>Submission ID</th>
            ...
            <tr v-for="submission in filteredSubmissions" :key="submission.id">
              ...
              <td>
                <div class="action-select-wrapper">
                  <select v-model="submission.status" class="action-select">
                  ...
                  <button @click="updateStatus(submission)" class="update-btn">Save</button>
                </div>
              </td>
            </tr>
          </tbody>
  ...
  try {
    ...
    if (response.data.success) {
      submissions.value = response.data.data.map(sub => ({
        ...
        eventTitle: sub.event?.title || '—'  // fallback if event not loaded
      }))
    } else {
      submissions.value = []
...
const updateStatus = async (submission) => {
  try {
    const token = localStorage.getItem('authToken')
    const response = await api.post(`/submissions/${submission.id}/status`, {
...
const filteredSubmissions = computed(() => {
  return submissions.value.filter(sub => {
    ...
    if (eventFilter.value && !sub.eventTitle.toLowerCase().includes(eventFilter.value.toLowerCase())) return false
    return true
  })
})
